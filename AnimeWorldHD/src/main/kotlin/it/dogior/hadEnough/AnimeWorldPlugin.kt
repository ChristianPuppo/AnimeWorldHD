...
@CloudstreamPlugin
class AnimeWorldHDPlugin : Plugin() {
    val sharedPref = activity?.getSharedPreferences("AnimeWorldHDIT", Context.MODE_PRIVATE)
    ...
    override fun load(context: Context) {
        ...
        if (isSplit) {
            if (dubEnabled) {
                registerMainAPI(AnimeWorldHDDub(isSplit))
            }
            if (subEnabled) {
                registerMainAPI(AnimeWorldHDSub(isSplit))
            }
        } else {
