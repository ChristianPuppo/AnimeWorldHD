...
open class AnimeWorldCore(isSplit: Boolean = false) : MainAPI() {
    ...
    override var name = "AnimeWorldHD"
...
//        Log.d("AnimeWorld:load", "NextAiring: $nextAiringDate $nextAiringTime")
        ...
        // Fetch high-quality images from ani.zip + AniList (same as Ultima)
        // Priority: ani.zip Fanart > AniList API > AnimeWorld scraping
        val aniListImages = anlId?.let { 
            try {
                AniListMetadataFetcher.fetchImagesById(it)
            } catch (e: Exception) {
                null
            }
        }
        
        // Use best available images with fallback chain
        val finalPoster = aniListImages?.getBestPoster() ?: poster
        val finalBanner = aniListImages?.getBestBackground()
        
        return newAnimeLoadResponse(title, actualUrl, type) {
            engName = title
            japName = otherTitle
            addPoster(finalPoster)
            // Add background banner if available (Fanart or AniList banner)
            finalBanner?.let { this.backgroundPosterUrl = it }
            this.year = year
